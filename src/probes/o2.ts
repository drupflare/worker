 ...
 * INITIAL_MEMORY=96MB, ENVIRONMENT=worker.
 *
 * 96MB, not the 64MB this said until 2026-08-11: measured at instantiation as
 * 100,663,296 bytes / 1536 pages, and `build/rc/control.rc` sets `INITIAL_MEMORY=96MB`.
 * 64MB is `static-free-v1`'s value, which is the binary most of this document's
 * memory figures came from.
 *
			...
			switch (url.pathname) {
				...
				case '/evict': {
					...
					return Response.json({
						...
						linearBefore: before,
						linearAfter: after,
						stillWorks: { status: ok.status, bytes: ok.bytes, error: ok.error ?? null }
					});
				}
...
$serve = function($path, $uid = 0) use ($al) {
  ...
  return $k->handle($req, \\Symfony\\Component\\HttpKernel\\HttpKernelInterface::MAIN_REQUEST, false);
};
