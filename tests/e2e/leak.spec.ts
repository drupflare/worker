 ...
 *
 * A step that gives up at 503 carries a warming body with no markers, which reads as an identity
 * that saw DIFFERENT content rather than none. The caller asserts the status separately.
 */
...
describe.skipIf(skip)('the identity leak differential', () => {
	...
	it('every identity sees the same markers, so no per-identity state reached a shared page', async () => {
		...
		// every step answered before any are compared: a warming body has no markers, and an empty
		// marker set is indistinguishable from an identity shown something else
