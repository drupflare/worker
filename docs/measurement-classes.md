...
emits one JSON document; `scripts/measure/metrics-gate.ts` compares it to `metrics/baseline.json`
and exits non-zero when a metric crosses its tolerance.
...
- **The gate fails when no check could be evaluated.** A run where everything skipped is
  ...
  no step.
- **Tolerances live in code, not in the baseline.** `--write-baseline` moves recorded values and
  cannot widen what is allowed. The baseline is data; `CHECKS` is the contract.
...
bump moves either one, the gate reports it, and the baseline refresh records who moved it.
...
bun run metrics:gate --metrics=dist/metrics.json --write-baseline
```

A clean checkout can produce the driver pack, the envelope and the test counts. The bundle, the
index audit and the pack shape need `.interp/` and `assets/`, which arrive with `bun run hydrate`;
without them those three report themselves as not collected and the gate says so in its summary.

Refreshing the baseline is a deliberate act with a reason attached to the commit. A run on a machine
that could not measure a metric keeps the recorded value rather than erasing it, so a refresh from a
partially hydrated tree cannot silently drop the bundle figure.
