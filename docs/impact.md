...
month with five saves a day, that is 28.34% of requests rendering for a CDN and origin against
0.004% for drupflare, 186,371 renders a day against 25. A per-location cache miss reaches the page
store, which answers without PHP, and a stored page goes stale only when a save invalidates one of
its tags.
...
a whole fleet is wrong in both directions. For a CDN and origin a common 1% assumption is first
reached at roughly 723 million views per site per month, far above the range these models cover.

Drupflare has no TTL term and no location term. Its renders are saves times the pages each save
invalidates, 5 for an ordinary node save, so at five saves a day a site renders about 25 pages a
day whatever its traffic. The fraction is 7.6% at 10,000 views a month, 1% at about 76,000 and
0.004% at 20 million. A save the main menu references invalidates every cached page, 34 on the
measured site, and is the pessimistic arm.

Two consequences for anyone reproducing this work. For a CDN and origin the edge-location count is
the largest term: at 20 million views it spans 4.38% at one location to 62.91% at fifty. And any
measurement taken from a single source address reads a floor, because a one-address generator
drives one edge location while real visitors spread across many.
...
behind a CDN, each arm on its own render fraction, generated by `bun scripts/economics/fleet.ts`:

| views/site/month | shared hosting renders | drupflare renders | saving |
| ---------------: | ---------------------: | ----------------: | -----: |
|           10,000 |                100.00% |             7.60% | 99.95% |
|          100,000 |                100.00% |             0.76% | 99.84% |
|        1,000,000 |                 83.36% |             0.08% | 98.98% |
|       20,000,000 |                 28.34% |            0.004% | 92.67% |
...
fill writes 1.75 rows, and the re-render ceiling is 50,916 a day, set by rows written. Renders follow
saves rather than views: at five saves a day a site re-renders about 25 pages, so a thousand active
sites use under half of that ceiling. For a large idle fleet the binding meter changes to storage, at 4.7 MB
...
| 10,000        | free   | free     | free      | $5.80       |
| 100,000       | free   | free     | $5.15     | $38.44      |
| 1,000,000     | free   | $5.15    | $37.64    | $365.14     |
| 10,000,000    | $5.15  | $37.56   | $364.34   | $3,644.64   |
...
| ------------- | ----------- | ------------ | ------------- | -------------- |
| 1,000         | free        | free         | free          | free           |
| 10,000        | free        | free         | free          | 862.3x cheaper |
| 100,000       | free        | free         | 97.1x cheaper | 130.1x cheaper |
| 1,000,000     | free        | 9.7x cheaper | 13.3x cheaper | 13.7x cheaper  |
| 10,000,000    | 1.0x dearer | 1.3x cheaper | 1.4x cheaper  | 1.4x cheaper   |
...
| ------------- | ------------ | ------------- | -------------- | --------------- |
| 1,000         | free         | free          | free           | free            |
| 10,000        | free         | free          | free           | 5174.0x cheaper |
| 100,000       | free         | free          | 582.5x cheaper | 780.5x cheaper  |
| 1,000,000     | free         | 58.3x cheaper | 79.7x cheaper  | 82.2x cheaper   |
| 10,000,000    | 5.8x cheaper | 8.0x cheaper  | 8.2x cheaper   | 8.2x cheaper    |
...
| 100,000       | 24.9 kWh | 248.8 kWh | 2.49 MWh  | 24.88 MWh   |
| 1,000,000     | 25.5 kWh | 255.1 kWh | 2.55 MWh  | 25.51 MWh   |
| 10,000,000    | 31.9 kWh | 318.7 kWh | 3.19 MWh  | 31.87 MWh   |
...
| 10    | one US home for 10.8 days  | 270 lb CO2e           | 4 lb of e-waste    |
| 100   | one US home for 107.9 days | 2,698 lb CO2e         | 40 lb of e-waste   |
| 1,000 | 3.0 US homes for a year    | 2.7 cars off the road | 397 lb of e-waste  |
...
the two, by the $5 subscription plus $0.15 of usage, and the latency-matched host is 5.8x dearer.
