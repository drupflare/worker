...
| 3   | serving 3.0M visits/month, regeneration 8,196 renders/day               | a     | **yes.** `tests/unit/free-envelope.spec.ts` over `scripts/measure/free-envelope.ts`.                                                                                                                                                                                                        |
...
| 10  | cold boot 1,398 ms, `page_cache` hit 1 ms, render 34 ms, startup 112 ms | c     | **no.** In-PHP `microtime()` and `Date.now()` do not ADVANCE on the edge, so a delta from either reads 0. The absolute is a real epoch; only the duration is unmeasurable there.                                                                                                            |
